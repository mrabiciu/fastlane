  ...
  class ConnectAPI
    class App
      ...
      def ensure_version!(version_string, platform: nil, client: nil)
        client ||= Spaceship::ConnectAPI
        puts("[DEBUG] Fetchilng app store version")
        app_store_version = get_edit_app_store_version(client: client, platform: platform)
        puts("[DEBUG] Did fetch app store version '#{app_store_version}'")
        ...
        if app_store_version
          ...
          puts("[DEBUG] Nothing to be done")
          return false
        else
          
          attributes = { versionString: version_string, platform: platform }
          puts("[DEBUG] No version found posting app store version '#{version_string}'")
          client.post_app_store_version(app_id: id, attributes: attributes)
          puts("[DEBUG] Did post app version")
          return true
        end
      end
